...
# Data
methods = ["OpenRewrite", "AgentCoder", "GPT-5.1", "TriArchitect"]
ssr = [62.0, 59.1, 65.2, 68.4]
hallucination = [0.0, 10.2, 8.1, 1.8]
cost = [0.00, 0.12, 0.09, 0.06]
types = ["Rule-Based", "Multi-Agent", "Single-Agent", "Multi-Agent"]
colors = ["#95a5a6", "#e74c3c", "#3498db", "#2ecc71"] # Grey, Red, Blue, Green
...
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval + 0.2, f"{yval}%", ha='center', va='bottom', fontsize=12, fontweight='bold')
plt.savefig(os.path.join(output_dir, "fig4_hallucination_scale.png"), dpi=100)
plt.close()

# Figure 5: Efficiency
plt.figure(figsize=(10, 6))
plt.scatter(cost, ssr, s=200, c=colors, edgecolors='black')
...
for i, txt in enumerate(methods):
    plt.annotate(txt, (cost[i]+0.005, ssr[i]), fontsize=11, fontweight='bold')
