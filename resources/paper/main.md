...
The Typed Migration Graph constitutes the **core technical contribution** enabling stateful migration. While iterative LLM correction is well-known (Reflexion [18]), the challenge in migration is *context*: single-agent approaches fail because they treat files in isolation.
...
Performs static analysis using `javalang` to populate the TMG. It identifies deprecated usage using a curated knowledge base (Appendix A) and builds the dependency graph.
...
- **Context Construction:** Queries TMG for migrated dependencies.
- **Structured Prompting:** Enforces TMG constraints (e.g., "Must use Jakarta").

### 4.3 Validator Agent
Provides runtime verification via **Docker**.
- **Isolation:** Each validation runs in a fresh container.
- **Verification:** Runs `mvn test`. Checks both compilation and test passage.
...
**[Figure 2: Consensus Flow]** *Proposal → Validator Execution → Decision (Veto/Commit) → Revision Loop.*
...
This simplifies the "Byzantine" problem: we trust the compiler and test suite implicitly.
...
**LLM-Based Agents.** Recent surveys [11] categorize agents into "Conversation-based" (ChatDev) and "Artifact-based". TriArchitect is artifact-based (TMG). We align with *AgentCoder* [8] but replace "Test Generation" with "Regression Testing".

**Hallucination Mitigation.** Unlike RAG approaches that reduce "Micro Hallucination Number" (MiHN) [3], TriArchitect eliminates "Dependency Drift" through topological ordering.
